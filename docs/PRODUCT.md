...
- **Distraction-Free Practice Header & Restrained Streak Feedback**: The permanent score HUD display has been removed from normal active practice. A restrained consecutive correct streak badge (e.g. `🔥 3`) appears in the metadata header during active practice only when the learner achieves $\ge 3$ consecutive correct answers in the current session. It disappears immediately upon an incorrect answer or timeout, with zero celebratory fanfare, XP, or gamification.
